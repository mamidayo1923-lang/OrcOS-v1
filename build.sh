...
if [ ! -d "limine" ]; then
    # Limine 8.xからはBIOSサポートが廃止されたため、BIOS対応の最終安定版である 7.x系 を取得します
    echo "Downloading Limine 7.x release..."
    URL=$(curl -s https://api.github.com/repos/limine-bootloader/limine/releases | grep "browser_download_url" | grep "limine-7\." | grep "\.tar\.xz" | head -n 1 | cut -d '"' -f 4)
    if [ -z "$URL" ]; then
        URL="https://github.com/limine-bootloader/limine/releases/download/v7.14.3/limine-7.14.3.tar.xz"
    fi
    wget -qO limine.tar.xz "$URL"
    mkdir -p limine
...
# Limine 7.x ではバイナリは limine/ 直下にあるので元に戻します
